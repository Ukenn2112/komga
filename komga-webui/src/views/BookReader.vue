  ...
  methods: {
    ...
    async setup(bookId: string, page?: number) {
      ...
      if (this.$route.query.contextId && this.$route.query.context
        ...
        document.title = `UCoin Manga - ${this.contextName} - ${this.book.metadata.title}`
      } else {
        document.title = `UCoin Manga - ${getBookTitleCompact(this.book.metadata.title, this.series.metadata.title)}`
      }
